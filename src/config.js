...
/** Web app base URL for ContactUrl links opened from 3CX (not the REST API host). */
function deriveAppUrl(apiUrl) {
    if (process.env.SMOKEBALL_APP_URL) {
        return process.env.SMOKEBALL_APP_URL.replace(/\/$/, '');
    }
    try {
